  ...
  methods: {
    ...
    async insertManureChemical(row) {
      try {
        ...
        this.manurechemicaldetails.push(response.data.data);
        if (response.statusText === "Created") {
          // If response status is 200 (OK), trigger success toast
          this.triggerToastMessage(
            "Inserted Manure Chemical Details Successfully",
            "custom_toast"
          );
        }
        ...
        this.triggerToastMessage(
          "Failed to Insert Manure Chemical Details",
          "danger"
        );
    ...
    async updateManureChemical(row) {
      try {
        ...
        if (response.statusText === "OK") {
          // If response status is 200 (OK), trigger success toast
          this.triggerToastMessage(
            "Updated Manure Chemical Details Successfully",
            "custom_toast"
          );
        }
        ...
        this.triggerToastMessage(
          "Failed to Update Manure Chemical Details",
          "danger"
        );
