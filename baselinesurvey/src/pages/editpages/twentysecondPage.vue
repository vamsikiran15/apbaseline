      ...
      <ion-card-content>
        ...
        >
          <ion-select-option value=""
            >Select Source of Income</ion-select-option
          >
  ...
  methods: {
    ...
    async insertAwarenessAdoption(row) {
      try {
        ...
        this.awarenessadoptiontechnologydetails.push(response.data.data);
        if (response.statusText === "Created") {
          // If response status is 200 (OK), trigger success toast
          this.triggerToastMessage(
            "Inserted AwarenessAdoption Details Successfully",
            "custom_toast"
          );
        }
        ...
        this.triggerToastMessage(
          "Failed to insert Awareness Adoption Technology Details",
          "danger"
        );
    ...
    async updateAwarenessAdoption(row) {
      try {
        ...
        if (response.statusText === "OK") {
          // If response status is 200 (OK), trigger success toast
          this.triggerToastMessage(
            "Updated AwarenessAdoption Details Successfully",
            "custom_toast"
          );
        }
        ...
        this.triggerToastMessage(
          "Failed to Update Awareness Adoption Technology Details",
          "danger"
        );
