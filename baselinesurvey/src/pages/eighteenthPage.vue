<template>
<ion-card>
    <ion-card-header color="tertiary" class="ion-text-center"><h3>18.Fodder/Fuel use during previous year(1 ton = 1000kgs, 1 Head load = 25kgs)</h3></ion-card-header>
</ion-card>
<ion-card>
    <ion-card-header color="tertiary" class="ion-text-center"><h6>Own Load</h6></ion-card-header>
    <ion-input
            class="ion-margin-top"
            placeholder="Enter value in Tons"
            fill="outline"
            label="Fodder Utilization(In Tons)"
            label-placement="floating"
          ></ion-input>
          <ion-input
            class="ion-margin-top"
            placeholder="Enter value"
            fill="outline"
            label="Fire/Fuel Cow Dung"
            label-placement="floating"
          ></ion-input>
          <ion-input
            class="ion-margin-top"
            placeholder="Enter values"
            fill="outline"
            label="Firewood"
            label-placement="floating"
          ></ion-input>
          <ion-input
            class="ion-margin-top"
            placeholder="Enter values"
            fill="outline"
            label="Agricultural Waste"
            label-placement="floating"
          ></ion-input>
</ion-card>
<ion-card>
    <ion-card-header color="tertiary" class="ion-text-center"><h6>Community</h6></ion-card-header>
    <ion-input
            class="ion-margin-top"
            placeholder="Enter value in Tons"
            fill="outline"
            label="Fodder Utilization(In Tons)"
            label-placement="floating"
          ></ion-input>
          <ion-input
            class="ion-margin-top"
            placeholder="Enter value"
            fill="outline"
            label="Fire/Fuel Cow Dung"
            label-placement="floating"
          ></ion-input>
          <ion-input
            class="ion-margin-top"
            placeholder="Enter values"
            fill="outline"
            label="Firewood"
            label-placement="floating"
          ></ion-input>
          <ion-input
            class="ion-margin-top"
            placeholder="Enter values"
            fill="outline"
            label="Agricultural Waste"
            label-placement="floating"
          ></ion-input>
</ion-card>
<ion-card>
    <ion-card-header color="tertiary" class="ion-text-center"><h6>Purchased</h6></ion-card-header>
    <ion-input
            class="ion-margin-top"
            placeholder="Enter value in Tons"
            fill="outline"
            label="Fodder Utilization(In Tons)"
            label-placement="floating"
          ></ion-input>
          <ion-input
            class="ion-margin-top"
            placeholder="Enter value"
            fill="outline"
            label="Fire/Fuel Cow Dung"
            label-placement="floating"
          ></ion-input>
          <ion-input
            class="ion-margin-top"
            placeholder="Enter values"
            fill="outline"
            label="Firewood"
            label-placement="floating"
          ></ion-input>
          <ion-input
            class="ion-margin-top"
            placeholder="Enter values"
            fill="outline"
            label="Agricultural Waste"
            label-placement="floating"
          ></ion-input>
</ion-card>
<ion-card>
    <ion-card-header color="tertiary" class="ion-text-center"><h6>Others</h6></ion-card-header>
    <ion-input
            class="ion-margin-top"
            placeholder="Enter value in Tons"
            fill="outline"
            label="Fodder Utilization(In Tons)"
            label-placement="floating"
          ></ion-input>
          <ion-input
            class="ion-margin-top"
            placeholder="Enter value"
            fill="outline"
            label="Fire/Fuel Cow Dung"
            label-placement="floating"
          ></ion-input>
          <ion-input
            class="ion-margin-top"
            placeholder="Enter values"
            fill="outline"
            label="Firewood"
            label-placement="floating"
          ></ion-input>
          <ion-input
            class="ion-margin-top"
            placeholder="Enter values"
            fill="outline"
            label="Agricultural Waste"
            label-placement="floating"
          ></ion-input>
</ion-card>
<ion-card>
    <ion-input
            class="ion-margin-top"
            fill="outline"
            label="Total"
            label-placement="floating"
          ></ion-input>
</ion-card>
</template>
<script>
export default{}
</script>